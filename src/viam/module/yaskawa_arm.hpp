...
public:
  ...
  /// class.
  static std::vector<std::shared_ptr<ModelRegistration>>
  create_model_registrations(boost::asio::io_context &io_context);

  explicit YaskawaArm(Model model, const Dependencies &deps,
                      const ResourceConfig &cfg,
                      boost::asio::io_context &io_context);
  ~YaskawaArm() override;

  void reconfigure(const Dependencies &deps,
                   const ResourceConfig &cfg) override;
  ...
  void move_through_joint_positions(
      ...
      const MoveOptions &options, const viam::sdk::ProtoStruct &extra) override;
