...
private:
  ...
  struct PendingPopOperation {
    boost::asio::any_completion_handler<void(std::optional<T>,
  ...
  void notify_one() {
    if (!pending_.empty() && !queue_.empty()) {
      ...
      queue_.pop();
      boost::asio::post(executor_, [handler = std::move(op.handler_),
...
public:
  ...
  template <typename CompletionToken> auto async_pop(CompletionToken &&token) {
    if (closed_)
      throw std::runtime_error("cannot pop on closed queue");
    return boost::asio::async_initiate<
        CompletionToken, void(std::optional<T>, boost::system::error_code ec)>(
        [this](auto &&handler) mutable {
          ...
          if (!queue_.empty()) {
            ...
            boost::asio::post(executor_, [handler = std::move(handler),
                                          item = std::move(item)]() mutable {
              handler(std::make_optional(std::move(item)),
                      boost::system::error_code());
            });
          } else {
            pending_.push(
                PendingPopOperation{std::forward<decltype(handler)>(handler)});
          }
        },
        token);
  }
  ...
  void clear() {
    ...
    while (!pending_.empty()) {
      ...
      boost::asio::post(
          executor_, [handler = std::move(ops.handler_)]() mutable {
...
public:
  RobotSocketBase(boost::asio::io_context &io_context, const std::string &host,
                  uint16_t port)
...
public:
  TcpRobotSocket(boost::asio::io_context &io_context,
...
public:
  explicit UdpBroadcastListener(boost::asio::io_context &io_context,
                                uint16_t port = 21789);
...
class YaskawaController
    : public std::enable_shared_from_this<YaskawaController> {
public:
  explicit YaskawaController(boost::asio::io_context &io_context, double speed,
                             double acceleration,
                             const std::string &host = "127.0.0.1");
...
private:
  ...
  Message
  create_status_response_from_cache(message_type_t requested_type) const;
  std::future<Message> make_goal_(std::list<Eigen::VectorXd> waypoints,
                                  const std::string &unix_time);
  std::future<Message>
  send_goal_(uint32_t group_index, uint32_t axes_controlled,
...
class GoalRequestHandle {
public:
  GoalRequestHandle(int32_t goal_id, std::shared_ptr<YaskawaController> robot,
