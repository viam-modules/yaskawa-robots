...
std::string_view viam_error_to_string(viam_error_code_t code) {
    switch (code) {
        ...
        case VIAM_ERROR_TRAJ_START_POS:
            return "trajectory start position doesn't match current robot position — query joint positions and start the trajectory from "
                   "there";
            ...
            while (true) {
                ...
                if (iteration++ % goal_status_polling_trigger == 0) {
                    ...
                    switch (status_msg.state) {
                        ...
                        case GOAL_STATE_ABORTED:
                            throw std::runtime_error(std::format("goal aborted: {}", viam_error_to_string(status_msg.abort_code)));
                    }
                }
            }
