...
void sampling_func(std::vector<trajectory_point_t> &samples,
                   double duration_sec, double sampling_frequency_hz,
                   const Func &f) {
  if (duration_sec <= 0.0 || sampling_frequency_hz <= 0.0) {
    throw std::invalid_argument(
        "duration_sec and sampling_frequency_hz are not both positive");
  }
  ...
  if (!std::isfinite(putative_samples) || putative_samples > k_max_samples) {
    throw std::invalid_argument("duration_sec and sampling_frequency_hz exceed "
                                "the maximum allowable samples");
  }
  // Calculate the number of samples needed. this will always be at least 2.
  const auto num_samples =
      static_cast<std::size_t>(std::ceil(putative_samples) + 1);
...
CartesianPosition::CartesianPosition(const Message &msg) {
  ...
  if (!(msg.header.message_type == MSG_GET_CART ||
        msg.header.message_type == MSG_FROM_JOINT_TO_CART))
  ...
  if (msg.payload.size() != sizeof(cartesian_payload_t))
    throw std::runtime_error(std::format(
        "incorrect cartesian payload size: expected {} bytes, got {} bytes",
        sizeof(cartesian_payload_t), msg.payload.size()));

  // Safe deserialization: verify alignment before reinterpret_cast
  if (reinterpret_cast<uintptr_t>(msg.payload.data()) %
          alignof(cartesian_payload_t) !=
      0)
    throw std::runtime_error("cartesian payload data is not properly aligned");

  const cartesian_payload_t *cart_coord =
      reinterpret_cast<const cartesian_payload_t *>(msg.payload.data());
...
AnglePosition::AnglePosition(const Message &msg) {
  ...
  if (!(msg.header.message_type == MSG_FROM_CART_TO_JOINT))
    throw std::runtime_error(std::format(
        "wrong message status type expected MSG_FROM_CART_TO_JOINT had {}",
        msg.header.message_type));

  // Validate payload size to prevent buffer overruns
  if (msg.payload.size() != sizeof(position_angle_degree_payload_t))
    throw std::runtime_error(std::format(
        "incorrect angle payload size: expected {} bytes, got {} bytes",
        sizeof(position_angle_degree_payload_t), msg.payload.size()));

  // Safe deserialization: verify alignment before reinterpret_cast
  if (reinterpret_cast<uintptr_t>(msg.payload.data()) %
          alignof(position_angle_degree_payload_t) !=
      0)
    throw std::runtime_error("angle payload data is not properly aligned");

  const position_angle_degree_payload_t *retPos =
      reinterpret_cast<const position_angle_degree_payload_t *>(
          msg.payload.data());
  pos.reserve(8);
  boost::copy(retPos->positionAngleDegree, std::back_inserter(pos));
}
AnglePosition::AnglePosition(std::vector<double> posRad) {
  std::transform(posRad.begin(), posRad.end(), posRad.begin(),
                 [](double d) -> double { return d * (180.0 / M_PI); });
  pos.reserve(8);
...
std::string AnglePosition::toString() noexcept {
  ...
  if (pos.size() < 6) {
    return std::format(
        "AnglePosition[invalid: only {} dimensions, expected at least 6]",
        pos.size());
  }

  // Format the first 6 joint angles (standard for 6-axis robot)
  return std::format(
      "AnglePosition[{:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}] (degrees)",
...
StatusMessage::StatusMessage(const Message &msg) {
  if (msg.header.message_type != MSG_ROBOT_POSITION_VELOCITY_TORQUE)
    throw std::runtime_error(std::format(
        "wrong message status type expected {} had {}",
        (int)MSG_ROBOT_POSITION_VELOCITY_TORQUE, msg.header.message_type));
  ...
  std::memcpy(&num_axes, msg.payload.data() + sizeof(timestamp),
              sizeof(num_axes));
  boost::span<const double> arrays{
      reinterpret_cast<const double *>(msg.payload.data() + sizeof(timestamp) +
                                       sizeof(num_axes)),
      4 * MAX_AXES};
  position.reserve(MAX_AXES);
  boost::copy(arrays | boost::adaptors::sliced(0, MAX_AXES),
              std::back_inserter(position));

  velocity.reserve(MAX_AXES);
  boost::copy(arrays | boost::adaptors::sliced(MAX_AXES, 2 * MAX_AXES),
              std::back_inserter(velocity));

  torque.reserve(MAX_AXES);
  boost::copy(arrays | boost::adaptors::sliced(2 * MAX_AXES, 3 * MAX_AXES),
              std::back_inserter(torque));
  position_corrected.reserve(MAX_AXES);
  boost::copy(arrays | boost::adaptors::sliced(3 * MAX_AXES, 4 * MAX_AXES),
...
RobotStatusMessage::RobotStatusMessage(const Message &msg) {
  if (msg.header.message_type != MSG_ROBOT_STATUS)
    throw std::runtime_error(
        std::format("wrong message status type expected {} had {}",
                    (int)MSG_ROBOT_STATUS, msg.header.message_type));
  if (msg.header.payload_length != msg.payload.size())
    throw std::runtime_error("incorrect status size");
  ...
  boost::span<const int> alarm{reinterpret_cast<const int *>(data),
                               MAX_ALARM_COUNT + 1};
  boost::copy(alarm | boost::adaptors::sliced(0, MAX_ALARM_COUNT + 1),
              std::back_inserter(error_codes));
...
Message::Message(Message &&msg) noexcept
    : header(std::move(msg.header)), payload(std::move(msg.payload)) {}
...
std::ostream &operator<<(std::ostream &os, const Message &msg) {
  os << "Message{type=" << static_cast<int>(msg.header.message_type)
     << ", timestamp=" << msg.header.timestamp_ms
     << ", payload_length=" << msg.header.payload_length << ", magic=0x"
     << std::hex << msg.header.magic_number << std::dec
     << ", version=" << static_cast<int>(msg.header.version) << "}";
  return os;
}

// TcpRobotSocket Implementation
TcpRobotSocket::TcpRobotSocket(boost::asio::io_context &io_context,
                               const std::string &host, uint16_t port)
    : RobotSocketBase(io_context, host, port), socket_(io_context),
      request_queue_(io_context.get_executor()) {}

TcpRobotSocket::~TcpRobotSocket() { disconnect(); }
...
awaitable<void> TcpRobotSocket::process_requests() {
  while (running_) {
    auto [result, error] =
        co_await request_queue_.async_pop(boost::asio::use_awaitable);
    if (error) {
    ...
    try {
      ...
      buffer.resize(sizeof(protocol_header_t) +
                    request_pair.first.payload.size());

      std::memcpy(buffer.data(), &request_pair.first.header,
                  sizeof(protocol_header_t));
      if (!request_pair.first.payload.empty()) {
        std::memcpy(buffer.data() + sizeof(protocol_header_t),
                    request_pair.first.payload.data(),
                    request_pair.first.payload.size());
      }
      ...
      size_t bytes_received = co_await socket_.async_read_some(
          boost::asio::buffer(header_buffer), use_awaitable);
      ...
      bytes_received = co_await socket_.async_read_some(
          boost::asio::buffer(payload_buffer), use_awaitable);
...
UdpRobotSocket::UdpRobotSocket(boost::asio::io_context &io_context,
                               State &state)
    : RobotSocketBase(io_context, "127.0.0.1", 0), robot_state_(state),
      socket_(io_context) {}

UdpRobotSocket::~UdpRobotSocket() { disconnect(); }
...
std::future<Message> UdpRobotSocket::send_request(Message /* request */) {
  ...
  auto future = promise.get_future();
  promise.set_exception(
      std::make_exception_ptr(std::runtime_error("UDP socket is read-only")));
  return future;
}
...
void UdpRobotSocket::get_status(std::promise<Message> promise) {
  ...
  cached_status_ = std::visit(
      [promise = std::move(promise)](auto &current) mutable
          -> std::variant<std::monostate, Message, std::promise<Message>> {
        ...
        if constexpr (std::is_same_v<Type, std::monostate>) {
          ...
          promise.set_exception(std::make_exception_ptr(std::runtime_error(
              "Concurrent status requests are not supported")));
  ...
  cached_robot_status_ = std::visit(
      [promise = std::move(promise)](auto &current) mutable
        ...
        if constexpr (std::is_same_v<Type, std::monostate>) {
          ...
          promise.set_exception(std::make_exception_ptr(std::runtime_error(
              "Concurrent robot status requests are not supported")));
          return std::move(current);
        }
      },
  ...
  while (running_) {
    try {
      ...
      size_t bytes_received = co_await socket_.async_receive_from(
          boost::asio::buffer(buffer), sender_endpoint, use_awaitable);
...
Message UdpRobotSocket::parse_message(const std::vector<uint8_t> &buffer) {
  ...
  if (message.header.magic_number != PROTOCOL_MAGIC_NUMBER ||
    ...
    throw std::runtime_error(std::format(
        "invalid message: wrong magic number or version expected magic : {:X} "
        "version: {} got magic: {:X} version: {}",
        PROTOCOL_MAGIC_NUMBER, PROTOCOL_VERSION,
        (int)message.header.magic_number, (int)message.header.version));
  }

  if (message.header.payload_length > 0) {
    if (buffer.size() <
        sizeof(protocol_header_t) + message.header.payload_length) {
      throw std::runtime_error(
          std::format("Invalid message: payload too short expected {} had {}",
    ...
    std::memcpy(message.payload.data(),
                buffer.data() + sizeof(protocol_header_t),
...
UdpBroadcastListener::UdpBroadcastListener(boost::asio::io_context &io_context,
                                           uint16_t port)
    : io_context_(io_context), socket_(io_context), port_(port) {}

UdpBroadcastListener::~UdpBroadcastListener() { stop(); }
...
boost::asio::awaitable<void> UdpBroadcastListener::receive_broadcasts() {
  while (running_) {
    try {
      ...
      std::size_t bytes_received = co_await socket_.async_receive_from(
          boost::asio::buffer(recv_buffer_), sender_endpoint, use_awaitable);
...
YaskawaController::YaskawaController(boost::asio::io_context &io_context,
    ...
    : io_context_(io_context), host_(host), robot_state_(State()),
...
std::future<void> YaskawaController::connect() {
  return std::async(std::launch::async, [this]() {
    try {
      ...
      LOGGING(info) << "UDP port registration response: "
...
std::future<Message> YaskawaController::get_goal_status(int32_t goal_id) {
  ...
  cancel_goal_payload_t *req =
      reinterpret_cast<cancel_goal_payload_t *>(payload.data());
  req->goal_id = goal_id;
...
std::future<Message> YaskawaController::setMotionMode(uint8_t mode) {
  ...
  motion_mode_payload_t *req =
      reinterpret_cast<motion_mode_payload_t *>(payload.data());
...
std::future<Message> YaskawaController::send_test_trajectory() {
  if (!robot_state_.IsReady()) {
    throw std::runtime_error(std::format(
        "cannot send test trajectory the robot state is e_stopped {} in error "
        "{}",
        robot_state_.e_stopped.load(), robot_state_.in_error.load()));
  }
...
std::future<Message> YaskawaController::turn_servo_power_on() {
  if (!robot_state_.IsReady()) {
    throw std::runtime_error(std::format(
        "cannot turn power on the robot state is e_stopped {} in error {}",
        robot_state_.e_stopped.load(), robot_state_.in_error.load()));
  }
...
std::future<Message> YaskawaController::get_robot_position_velocity_torque() {
  ...
  if (!udp_socket_) {
    promise.set_exception(std::make_exception_ptr(
        std::runtime_error("UDP socket not connected")));
    return future;
  }
...
std::future<Message> YaskawaController::register_udp_port(uint16_t port) {
  ...
  udp_port_registration_payload_t *port_payload =
      reinterpret_cast<udp_port_registration_payload_t *>(payload.data());
...
std::future<Message>
YaskawaController::send_goal_(uint32_t group_index, uint32_t axes_controlled,
                              const std::vector<trajectory_point_t> &trajectory,
                              const std::vector<tolerance_t> &tolerance) {
  std::vector<uint8_t> payload;
  payload.reserve(sizeof(uint32_t) * 2 +
                  trajectory.size() * sizeof(trajectory_point_t) +
...
std::unique_ptr<GoalRequestHandle>
YaskawaController::move(std::list<Eigen::VectorXd> waypoints,
                        const std::string &unix_time) {
  if (!robot_state_.IsReady()) {
    reset_errors().get();
    throw std::runtime_error(std::format(
        "cannot move the robot state is e_stopped {} in error {}",
        robot_state_.e_stopped.load(), robot_state_.in_error.load()));
  }
  ...
  if (response.header.message_type != MSG_GOAL_ACCEPTED) {
    throw std::runtime_error(std::format("Expected MSG_GOAL_ACCEPTED, got {}",
                                         (int)response.header.message_type));
  }

  if (response.payload.size() != sizeof(goal_accepted_payload_t)) {
    throw std::runtime_error(std::format(
        "Invalid goal accepted payload size expected {} got {}",
        sizeof(goal_accepted_payload_t), (int)response.payload.size()));
  }
  const goal_accepted_payload_t *accepted =
      reinterpret_cast<const goal_accepted_payload_t *>(
          response.payload.data());
  auto promise = std::promise<goal_state_t>();
  auto handle = std::make_unique<GoalRequestHandle>(
      accepted->goal_id, shared_from_this(), promise.get_future());

  std::thread([promise = std::move(promise), self = shared_from_this(),
               goal_id = accepted->goal_id]() mutable {
    try {
      while (1) {
        auto status_msg =
            GoalStatusMessage(self->get_goal_status(goal_id).get());
        ...
        if (status_msg.state == GOAL_STATE_SUCCEEDED) {
          ...
          throw std::runtime_error(
              std::format("goal failed - goal status is {}",
                          goal_state_to_string(status_msg.state)));
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
      }
...
std::future<Message>
YaskawaController::make_goal_(std::list<Eigen::VectorXd> waypoints,
                              const std::string &unix_time) {
  LOGGING(info) << "move: start unix_time_ms " << unix_time
                << " waypoints size " << waypoints.size();

  auto curr_joint_pos =
      StatusMessage(get_robot_position_velocity_torque().get()).position;

  auto curr_waypoint_rad = Eigen::VectorXd::Map(
      curr_joint_pos.data(),
      boost::numeric_cast<Eigen::Index>(curr_joint_pos.size()));
  if (!curr_waypoint_rad.isApprox(waypoints.front(),
                                  k_waypoint_equivalancy_epsilon_rad)) {
  ...
  LOGGING(info) << "generating trajectory with max speed: "
                << radians_to_degrees(max_velocity[0]);

  std::vector<trajectory_point_t> points;

  const Trajectory trajectory(Path(waypoints, 0.1), max_velocity,
                              max_acceleration);
  ...
  if (!std::isfinite(duration)) {
    throw std::runtime_error(
        "trajectory.getDuration() was not a finite number");
  }
  ...
  if (duration < k_min_timestep_sec) {
    throw std::runtime_error(
        std::format("duration {} lower than {} assuming arm is at goal",
                    duration, k_min_timestep_sec));
  }
  ...
  sampling_func(
      points, duration, k_sampling_freq_hz, [&](const double t, const double) {
        auto p_eigen = trajectory.getPosition(t);
        auto v_eigen = trajectory.getVelocity(t);
        auto secs = std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::duration<double>(t));
        auto nanos = std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::duration<double>(t) - secs);

        return trajectory_point_t{
            {p_eigen[0], p_eigen[1], p_eigen[2], p_eigen[3], p_eigen[4],
             p_eigen[5]},
            {v_eigen[0], v_eigen[1], v_eigen[2], v_eigen[3], v_eigen[4],
             v_eigen[5]},
            {0},
            {0},
  ...
  LOGGING(info) << "move: compute_trajectory end " << unix_time
                << " samples.size() " << points.size() << " segments "
                << waypoints.size() - 1;
...
std::future<Message> YaskawaController::cartPosToAngle(CartesianPosition &pos) {
  ...
  cartesian_payload_t *cid =
      reinterpret_cast<cartesian_payload_t *>(payload.data());
...
std::future<Message> YaskawaController::angleToCartPos(AnglePosition &pos) {
  ...
  position_angle_degree_payload_t *pid =
      reinterpret_cast<position_angle_degree_payload_t *>(payload.data());
...
GoalStatusMessage::GoalStatusMessage(const Message &msg) {
  if (msg.header.message_type != MSG_GOAL_STATUS)
    throw std::runtime_error(
        std::format("wrong message type expected {} had {}",
                    (int)MSG_GOAL_STATUS, msg.header.message_type));
  if (msg.payload.size() != sizeof(goal_status_payload_t))
    throw std::runtime_error(
        std::format("incorrect goal status payload size expected {} had {}",
                    sizeof(goal_status_payload_t), msg.payload.size()));

  const goal_status_payload_t *payload =
      reinterpret_cast<const goal_status_payload_t *>(msg.payload.data());
...
GoalRequestHandle::GoalRequestHandle(
    int32_t goal_id, std::shared_ptr<YaskawaController> controller,
    std::shared_future<goal_state_t> completion_future)
    : goal_id_(goal_id), is_done_(false), controller_(controller),
      completion_future_(completion_future) {}

goal_state_t GoalRequestHandle::wait() { return completion_future_.get(); }

std::optional<goal_state_t>
GoalRequestHandle::wait_for(std::chrono::milliseconds timeout) {
...
std::future<GoalStatusMessage> GoalRequestHandle::get_status() {
  return std::async(std::launch::async, [this]() mutable -> GoalStatusMessage {
    ...
    if (response.header.message_type == MSG_ERROR) {
      throw std::runtime_error(std::format(
          "received an error message while getting status for goal id {}",
          goal_id_));
    }
...
void GoalRequestHandle::cancel() {
  ...
  if (response.header.message_type == MSG_ERROR) {
    throw std::runtime_error(std::format(
        "received an error message while cancelling  goal id {}", goal_id_));
  }
}
...
State::State()
    : e_stopped(false), in_motion(false), drive_powered(false), in_error(true) {
}
...
bool State::IsReady() { return !e_stopped.load() && !in_error.load(); }
} // namespace robot
