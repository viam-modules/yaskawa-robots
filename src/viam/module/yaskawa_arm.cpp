...
pose cartesian_position_to_pose(CartesianPosition &&pos) {
  auto q = std::unique_ptr<void, decltype(&free_quaternion_memory)>(
      quaternion_from_euler_angles(degrees_to_radians(pos.rx),
                                   degrees_to_radians(pos.ry),
                                   degrees_to_radians(pos.rz)),
      &free_quaternion_memory);
  auto ov = std::unique_ptr<void, decltype(&free_orientation_vector_memory)>(
      orientation_vector_from_quaternion(q.get()),
      &free_orientation_vector_memory);

  auto components =
      std::unique_ptr<double[], decltype(&free_orientation_vector_components)>(
          orientation_vector_get_components(ov.get()),
          &free_orientation_vector_components);
  auto position = coordinates{pos.x, pos.y, pos.z};
  auto orientation =
      pose_orientation{components[0], components[1], components[2]};
...
std::string unix_time_iso8601() {
  ...
  const auto tt = chrono::system_clock::to_time_t(
      chrono::system_clock::time_point{seconds_part});
  const auto delta_us = chrono::duration_cast<chrono::microseconds>(
      now.time_since_epoch() - seconds_part);
...
std::vector<std::string> validate_config_(const ResourceConfig &cfg) {
  ...
  if (!find_config_attribute<double>(cfg, "acceleration_rad_per_sec2")) {
    throw std::invalid_argument(
        "attribute `acceleration_rad_per_sec2` is required");
  }

  auto threshold =
      find_config_attribute<double>(cfg, "reject_move_request_threshold_rad");
  constexpr double k_min_threshold = 0.0;
  constexpr double k_max_threshold = 2 * M_PI;
  if (threshold &&
      (*threshold < k_min_threshold || *threshold > k_max_threshold)) {
    std::stringstream sstream;
    sstream
        << "attribute `reject_move_request_threshold_rad` should be between "
        << k_min_threshold << " and " << k_max_threshold
...
std::vector<std::shared_ptr<ModelRegistration>>
YaskawaArm::create_model_registrations(boost::asio::io_context &io_context) {
  ...
  const auto registration_factory = [&](auto m) {
    ...
    return std::make_shared<ModelRegistration>(
        ...
        [&, model](auto deps, auto config) {
          return std::make_shared<YaskawaArm>(model, deps, config, io_context);
        },
        [](auto const &config) { return validate_config_(config); });
  };

  auto registrations =
      model_strings | boost::adaptors::transformed(registration_factory);
  return {std::make_move_iterator(begin(registrations)),
          std::make_move_iterator(end(registrations))};
}

YaskawaArm::YaskawaArm(Model model, const Dependencies &deps,
                       const ResourceConfig &cfg,
                       boost::asio::io_context &io_context)
    : Arm(cfg.name()), model_(std::move(model)), io_context_(io_context) {
  VIAM_SDK_LOG(info) << "Yaskawa Arm constructor called (model: "
                     << model_.to_string() << ")";

  configure_(deps, cfg);
}

void YaskawaArm::configure_(const Dependencies &,
  ...
  resource_root_ = std::filesystem::canonical(module_executable_directory /
                                              k_relpath_bindir_to_datadir /
                                              "yaskawa-robots");
  VIAM_SDK_LOG(info) << "Yaskawa robots module executable found in `"
                     << module_executable_path
                     << "; resources will be found in `" << resource_root_
                     << "`";

  threshold_ = find_config_attribute<double>(
      config, "reject_move_request_threshold_rad");

  auto speed =
      find_config_attribute<double>(config, "speed_rad_per_sec").value();
  auto acceleration =
      find_config_attribute<double>(config, "acceleration_rad_per_sec2")
          .value();

  robot_ = std::make_shared<YaskawaController>(io_context_, speed, acceleration,
                                               host);
  ...
  while (connection_try < k_max_connection_try) {
    try {
      ++connection_try;
      robot_->connect().get();
      VIAM_SDK_LOG(info) << "Successfully connected to robot on attempt "
                         << connection_try;
      break; // Exit on successful connection
    } catch (std::exception &ex) {
      VIAM_SDK_LOG(error) << std::format(
          "connection {} out of {} failed because {}", connection_try,
          k_max_connection_try, ex.what());
      if (k_max_connection_try == connection_try)
        throw;
    }
  }
}
void YaskawaArm::reconfigure(const Dependencies &deps,
                             const ResourceConfig &cfg) {
...
std::vector<double> YaskawaArm::get_joint_positions(const ProtoStruct &) {
  ...
  auto joint_rads =
      StatusMessage(robot_->get_robot_position_velocity_torque().get());
  auto joint_position_degree =
      joint_rads.position |
      boost::adaptors::transformed(radians_to_degrees<const double &>);
  return {std::begin(joint_position_degree), std::end(joint_position_degree)};
}

void YaskawaArm::move_through_joint_positions(
    const std::vector<std::vector<double>> &positions, const MoveOptions &,
  ...
  if (!positions.empty()) {
    ...
    for (const auto &position : positions) {
      auto next_waypoint_deg = Eigen::VectorXd::Map(
          position.data(), boost::numeric_cast<Eigen::Index>(position.size()));
      auto next_waypoint_rad =
          degrees_to_radians(std::move(next_waypoint_deg)).eval();

      // Skip waypoints that are too close to the previous one to avoid
      // redundant motion
      if ((!waypoints.empty()) &&
          (next_waypoint_rad.isApprox(waypoints.back(),
...
void YaskawaArm::move_to_joint_positions(const std::vector<double> &positions,
                                         const ProtoStruct &) {
  auto next_waypoint_deg =
      Eigen::VectorXd::Map(positions.data(),
                           boost::numeric_cast<Eigen::Index>(positions.size()))
          .eval();
...
YaskawaArm::KinematicsData YaskawaArm::get_kinematics(const ProtoStruct &) {
  ...
  if (!sva_file) {
    throw std::runtime_error(
        boost::str(boost::format("unable to open kinematics file '%1%'") %
                   sva_file_path.string()));
  }
  ...
  if (sva_file.bad()) {
    throw std::runtime_error(
        boost::str(boost::format("error reading kinematics file '%1%'") %
                   sva_file_path.string()));
  }
...
pose YaskawaArm::get_end_position(const ProtoStruct &) {
  ...
  auto p = cartesian_position_to_pose(
      CartesianPosition(robot_->getCartPosition().get()));
...
void YaskawaArm::stop(const ProtoStruct &) { robot_->stop().get(); }
...
YaskawaArm::~YaskawaArm() { robot_->disconnect(); }
