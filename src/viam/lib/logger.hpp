...
public:
  ...
  LogStream debug() { return log(LogLevel::DEBUG); }

  LogStream info() { return log(LogLevel::INFO); }

  LogStream warning() { return log(LogLevel::WARNING); }

  LogStream error() { return log(LogLevel::ERROR); }

  LogStream critical() { return log(LogLevel::CRITICAL); }
...
inline std::shared_ptr<ILogger>
make_logger(LogLevel min_level = LogLevel::INFO) {
...
inline std::shared_ptr<ILogger> get_global_logger() { return global_logger; }
