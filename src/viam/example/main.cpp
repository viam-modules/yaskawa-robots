...
public:
  explicit FileAndConsoleLogger(
      const std::string &filename,
      viam::yaskawa::LogLevel min_level = viam::yaskawa::LogLevel::INFO)
...
protected:
  void write_log(viam::yaskawa::LogLevel level,
...
private:
  std::string format_message(viam::yaskawa::LogLevel level,
...
generateCirclePosition(double r, double lb, double hb, int steps,
                       const CartesianPosition &seed) {
...
void example(asio::io_context &io_context) {
  auto robot =
      std::make_shared<YaskawaController>(io_context, 1.1, 1.1, "10.1.11.177");
  try {
    ...
    std::thread([=] {
      try {
        while (1) {
          ...
          StatusMessage pos_vel_msg(
              robot->get_robot_position_velocity_torque().get());
          std::cout << " Position [" << pos_vel_msg.position[0] << " "
                    << pos_vel_msg.position[1] << " " << pos_vel_msg.position[2]
                    << " " << pos_vel_msg.position[3] << " "
                    << pos_vel_msg.position[4] << " " << pos_vel_msg.position[5]
                    << "] Velocity [ " << pos_vel_msg.velocity[0] << " "
                    << pos_vel_msg.velocity[1] << " " << pos_vel_msg.velocity[2]
                    << " " << pos_vel_msg.velocity[3] << " "
                    << pos_vel_msg.velocity[4] << " " << pos_vel_msg.velocity[5]
                    << "]" << std::endl;
    ...
    for (int i = 0; i < 1; i++) {
      auto currentCartPositon =
          CartesianPosition(robot->getCartPosition().get());
      currentCartPositon.z -= 100;
      StatusMessage pos_vel_msg(
          robot->get_robot_position_velocity_torque().get());
      auto currentAnglePos = AnglePosition(pos_vel_msg.position);
      // currentAnglePos.toRad()
      auto currentAnglePosEigen =
          Eigen::VectorXd::Map(pos_vel_msg.position.data(),
                               (long)pos_vel_msg.position.size())
              .eval();
      auto circleWayPointCart = generateCirclePosition(
          1009.647, -M_PI / 2, M_PI, 20, currentCartPositon);
      auto circleWayPointAngle = std::list<Eigen::VectorXd>();
      std::transform(
          circleWayPointCart.begin(), circleWayPointCart.end(),
          std::back_inserter(circleWayPointAngle), [&](CartesianPosition &pos) {
            auto angle = AnglePosition(robot->cartPosToAngle(pos).get());
            angle.toRad();
            return Eigen::VectorXd::Map(angle.pos.data(),
                                        (long)angle.pos.size())
                .eval();
          });

      auto it = std::min_element(
          circleWayPointAngle.begin(), circleWayPointAngle.end(),
          [&](const auto &a, const auto &b) -> bool {
            return (a - currentAnglePosEigen)
                       .template lpNorm<Eigen::Infinity>() <
                   (b - currentAnglePosEigen)
                       .template lpNorm<Eigen::Infinity>();
          });
      ...
      std::copy(circleWayPointAngle.rbegin(), circleWayPointAngle.rend(),
                std::back_inserter(finalPoints));
      std::copy(circleWayPointAngle.begin(), it,
                std::back_inserter(finalPoints));
      finalPoints.push_back(currentAnglePosEigen);
      std::cout << "ccurrent pos " << currentAnglePos.toString() << " idx "
                << std::distance(circleWayPointAngle.begin(), it) << std::endl;
      ...
      for (auto &p : finalPoints) {
        std::cout << std::format(
                         ...
                         p[0] * (180.0 / M_PI), p[1] * (180.0 / M_PI),
                         p[2] * (180.0 / M_PI), p[3] * (180.0 / M_PI),
                         p[4] * (180.0 / M_PI), p[5] * (180.0 / M_PI))
                  << std::endl;
      }
...
int main() {
  try {
    ...
    auto custom_logger = std::make_shared<FileAndConsoleLogger>(
        "robot.log", viam::yaskawa::LogLevel::DEBUG);
